...
for problem in range(T):
    ...
    history = set()
    lvl = {}
    for top in range(max(start), max(start) + 200):
        state = (top,) * len(start)
        # print("step for {0}".format(state))
        # print("history size: {0}".format(len(history)))
        lvl[state] = 0
        q = queue.Queue()
        q.put(state)
        while not q.empty():
            element = q.get()
            new_elements = generate_new_elements(element)
            parent_lvl = lvl[element]

            for e in new_elements:
                if smaller_tuple(e, start) or False:
                    continue
                elif e in history:
                    continue
                elif e == start:
